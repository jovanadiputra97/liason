...
{
	insertItemStock($_REQUEST['nobarang'], $_REQUEST['jenisukuran'], $_REQUEST['stok']);
	$item = getItems(array("no" => " = ".$_REQUEST['nobarang']))->fetch(PDO::FETCH_ASSOC);
	insertLog(getLogCategoryNoByName('Tambah'), $_SESSION['userno'], getPageNoByName('+ Tambah Stok'), $_REQUEST['nobarang']." - ".$_REQUEST['jenisukuran']." : ".$_REQUEST['stok']);
	
?>
	<form name='FormSuccess' method='post' action='stok-lihat'><input type="hidden" name="notificationerror" value="0"><input type='hidden' name='notification' value='Sukses <?php echo ($_REQUEST['ams'] == 1 ? "merubah" : "menambahkan") ?> stok untuk <?php echo $item['code'] ?>'></form>
                    ...
                    <div class="card-body">
                      <form class="form-horizontal" name="FormAdd" id="FormAdd" method="post">
						...
						<div class="form-group row">
                          ...
                          <div class="col-sm-9">
                            <select name="jenisukuran" class="form-control selectpadding" required>
						...
						<div class="form-group row">
                          ...
                          <div class="col-sm-9">
                            <input type="number" class="form-control formauto displayinline" name="stok" required min="0.01" step="0.01"> lusin
                          </div>
                        </div>
